...
def parse_args():
    p = argparse.ArgumentParser(description="Alert if any ticker has N consecutive down days.")
    ...
    p.add_argument("--days_check", type=int, default=2, help="Consecutive down days threshold.")
...
def build_email_html(matches, last_n_to_show, n):
    if not matches:
        return None
    ...
    <html>
    <body>
      <p>The following tickers have <b>{n} consecutive down day(s)</b> (by daily close):</p>
      {table}
...
def send_email(subject, html_body, email_from, email_to, smtp_host, smtp_port, smtp_user, smtp_pass):
    ...
    text_fallback = "Your email client does not support HTML.\n\n" \
                    "Tickers matched the consecutive down-days rule."
...
def main():
    ...
    print(f"[INFO] Successfully fetched data for tickers: {list(closes_by_ticker.keys())}")
    ...
    if matches:
        print(f"[INFO] Tickers with {args.days_check} consecutive down days: {[t for t, _ in matches]}")
        now_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
        subject = f"[Stock Alert] {len(matches)} ticker(s) down {args.days_check} day(s) in a row — {now_utc}"
        html_body = build_email_html(matches, args.last_n_to_show, args.days_check)
        ...
        print(f"[INFO] Email sent for {len(matches)} ticker(s).")
    else:
        print(f"[INFO] No tickers with {args.days_check} consecutive down day(s) today.")
